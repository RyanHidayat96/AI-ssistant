...
public class MainActivity extends Activity {
    ...
    private String toolWorkspaceBootstrap(String wd) {
        ...
        return "cd " + wd + " 2>/dev/null; export WD=" + wd + "; export TOOLS=" + tools
                ...
                + "agent_tool_acquire() { [ \"$#\" -ge 4 ] && [ \"$#\" -le 5 ] || { echo 'usage: agent_tool_acquire name version abi url [filename]' >&2; return 2; }; "
                + "agent_tool_part \"$1\" && agent_tool_part \"$2\" && agent_tool_part \"$3\" || return $?; "
                + "case \"$4\" in http://*|https://*) ;; *) echo 'tool acquire: url must be http(s)' >&2; return 2;; esac; "
                + "D=$(agent_tool_shared \"$1\" \"$2\" \"$3\") || return $?; mkdir -p \"$D/_downloads\" || return $?; "
                + "F=${5:-${4##*/}}; F=${F%%\\?*}; [ -n \"$F\" ] || F=\"$1-$2.download\"; agent_tool_part \"$F\" || return $?; "
                + "OUT=\"$D/_downloads/$F\"; if [ -s \"$OUT\" ]; then echo \"cached_download=$OUT\"; echo \"dir=$D\"; return 0; fi; "
                + "TMP=\"$OUT.tmp.$$\"; rm -f \"$TMP\"; "
                + "if command -v curl >/dev/null 2>&1; then curl -L --fail --retry 2 -o \"$TMP\" \"$4\"; "
                + "elif command -v wget >/dev/null 2>&1; then wget -O \"$TMP\" \"$4\"; "
                + "else echo 'tool acquire: curl/wget missing; install a downloader through the smallest compatible path first' >&2; return 127; fi; "
                + "RC=$?; [ \"$RC\" -eq 0 ] || { rm -f \"$TMP\"; return \"$RC\"; }; mv \"$TMP\" \"$OUT\" && chmod 600 \"$OUT\"; "
                + "echo \"downloaded=$OUT\"; echo \"dir=$D\"; echo 'next: unpack/install only inside dir, run harmless --help/--version or sample test, then agent_tool_register_shared name version abi executable context'; }; "
