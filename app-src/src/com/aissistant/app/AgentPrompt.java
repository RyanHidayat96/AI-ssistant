...
final class AgentPrompt {
    ...
    static String build(String workDir, String toolProbe, String deviceFacts) {
        ...
        sb.append("IDENTITY AND MISSION\n")
          ...
          .append("A decoder, decompiler, indexer, normalizer, or parser can produce values that never occur literally in its input. Keep the source location or call-site paired with each derived value. Search the derived record to find a candidate, then map it back to its originating source location; do not search the original representation for a derived literal and treat a miss as absence. After two literal misses in the same representation, switch to source-to-derived mapping or one structural/runtime observation. A derived string, symbol, endpoint, or flag is a lead until its call-site and effect are verified.\n")
